#include <stdio.h>
#include <string.h>
#include <math.h>

#define WIDTH 80
#define HEIGHT 24
#define FRAMESIZE (WIDTH * HEIGHT)
#define THETA_STEP 0.05f
#define PHI_STEP 0.03f
#define R_MAJOR 2.0f
#define R_MINOR 1.0f

const char *shades = ".,-~:;=!*#$@";

int clamp(int val, int min, int max) {
    if (val < min) return min;
    if (val > max) return max;
    return val;
}

void project_point(float theta, float phi, float A, float B,
                   int *x_out, int *y_out, float *D_out) {

    float cos_theta = cosf(theta);
    float sin_theta = sinf(theta);
    float cos_phi = cosf(phi);
    float sin_phi = sinf(phi);
    float cos_A = cosf(A);
    float sin_A = sinf(A);
    float cos_B = cosf(B);
    float sin_B = sinf(B);

    float circle_x = R_MAJOR + R_MINOR * cos_theta;
    float circle_y = R_MINOR * sin_theta;

    float x3d = circle_x * (cos_B * cos_phi + sin_A * sin_B * sin_phi) - circle_y * cos_A * sin_B;
    float y3d = circle_x * (sin_B * cos_phi - sin_A * cos_B * sin_phi) + circle_y * cos_A * cos_B;
    float z3d = cos_A * circle_x * sin_phi + circle_y * sin_A + 5.0f;

    *D_out = 1.0f / z3d;
    *x_out = (int)(WIDTH / 2 + x3d * (*D_out) * 30);
    *y_out = (int)(HEIGHT / 2 + y3d * (*D_out) * 15);
}

int main() {
    float A = 0.0f;
    float B = 0.0f;

    while (1) {
        float zbuf[FRAMESIZE];
        char frame[FRAMESIZE];

        memset(frame, ' ', FRAMESIZE);
        memset(zbuf, 0, sizeof(zbuf));

        for (float theta = 0; theta < 2 * M_PI; theta += THETA_STEP) {
            for (float phi = 0; phi < 2 * M_PI; phi += PHI_STEP) {
                int x, y;
                float D;

                //should i try to add sampling

                project_point(theta, phi, A, B, &x, &y, &D);

                int idx = clamp(x, 0, WIDTH - 1) + WIDTH * clamp(y, 0, HEIGHT - 1);

                if (D > zbuf[idx]) {
                    zbuf[idx] = D;
                    int shade_idx = clamp((int)(D * 12), 0, 11);
                    frame[idx] = shades[shade_idx];
                }
            }
        }

        printf("\x1b[H");

        for (int i = 0; i < FRAMESIZE; i++) {
            putchar((i % WIDTH) ? frame[i] : '\n');
        }

        A += 0.04f;
        B += 0.02f;
    }

    return 0;
}
